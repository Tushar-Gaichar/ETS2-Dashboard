/**
 * Binary min-heap keyed by a separate cost array, used as the A* open-set
 * priority queue in algorithm.ts. Ported directly from TruckNav-Sim
 * (github.com/Rares-Muntean/TruckNav-Sim) — GPL-3.0.
 */
export class MinHeap {
  ...
  push(id: number, cost: number) {
    if (this.length >= this.ids.length) this.resize();
  ...
  private bubbleDown(index: number) {
    ...
    for (;;) {
      ...
      const right = left + 1;
      let smallest = index;
      if (left <= last && this.costs[left]! < this.costs[smallest]!) smallest = left;
      if (right <= last && this.costs[right]! < this.costs[smallest]!) smallest = right;
